...
#rbind joins vectors as rows to form a matrix
#cbind joins vectors as columns to form a matrix
rm(KobeBryant_PTS, JoeJohnson_PTS, LeBronJames_PTS, CarmeloAnthony_PTS, DwightHoward_PTS, ChrisBosh_PTS, ChrisPaul_PTS, KevinDurant_PTS, DerrickRose_PTS, DwayneWade_PTS)
#rm() removed memory
colnames(Points) <- Seasons
rownames(Points) <- Players
# rownames() and colnames() give the matrix in parenthesis the respective labels
...
#Function
myplot<-function(data_new,rows_=1:10){
  data<-data_new[rows_,,drop=F]
  matplot(t(data),type='b',pch=15:18,col=c(1:4,6))
  legend('bottomleft',inset =0.01,legend=Players[rows_],col=c(1:4,6),pch=15:18,horiz=F)
